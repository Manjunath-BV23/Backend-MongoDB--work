...
router.post("", async (req, res) => {
    ...
    router.get("", async (req, res) => {
      ...
      try {
        const users = await Submission.find().lean().exec(); // db.users.find() // proper promise
    ...
    router.get("/:id", async (req, res) => {
      try {
        const user = await Submission.findById(req.params.id).lean().exec();
    ...
    router.patch("/:id", async (req, res) => {
      try {
        const user = await Submission.findByIdAndUpdate(req.params.id, req.body, {
          new: true,
        })
          .lean()
          .exec();
